...
You can run a browser-targeted version:

1. Install dependencies: `npm install`
2. Start web server: `npm run start:web`
3. Open: `http://localhost:5173`

Notes for browser mode:
- Uses `src/browser.html` as the entrypoint.
- Screen capture works via `getDisplayMedia`.
- Desktop-only Electron features (always-on-top, global shortcuts, content-protection hooks, native audio helpers) are not available.
